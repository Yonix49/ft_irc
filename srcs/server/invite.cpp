...
/*   Updated: 2023/11/07 11:21:22 by kgezgin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::invite(std::string param, int fd)
{
	...
	{
		sendOneRPL(ERR_NOSUCHNICK(user->getNickname(), cmdLine[1]), fd);
		return ;
	}
	...
	{
		sendOneRPL(ERR_CHANOPRIVSNEEDED(cmdLine[2], user->getNickname()), fd);
		return ;
	}
